...
describe("updateSummaryAction", () => {
  it("PATCHes the log + revalidates the elog path on success", async () => {
    updateFlightLog.mockResolvedValueOnce({ id: "log-1" });

    const result = await updateSummaryAction("log-1", {
  ...
  it("forwards null to clear a counter (explicit not-entered)", async () => {
    updateFlightLog.mockResolvedValueOnce({ id: "log-1" });
    const result = await updateSummaryAction("log-1", { holds: null });
    expect(result.status).toBe("ok");
    expect(updateFlightLog).toHaveBeenCalledWith("log-1", { holds: null });
  });

  it("maps 409 submitted-lockdown to a friendly message", async () => {
    ...
    const result = await updateSummaryAction("log-1", { holds: 1 });
  ...
  it("maps 422 to the out-of-range explainer", async () => {
    updateFlightLog.mockRejectedValueOnce(
      new TestApiError(
        422,
        "/ops/flight-logs/log-1",
        JSON.stringify({ detail: [{ loc: ["body"], msg: "ge" }] }),
      ),
    );
    ...
    expect(result.status).toBe("error");
    if (result.status === "error") {
      expect(result.message).toMatch(/0 or higher/i);
    }
  });
  ...
  it("maps 404 flight_log_not_found to a reload nudge", async () => {
    updateFlightLog.mockRejectedValueOnce(
      new TestApiError(
        404,
        "/ops/flight-logs/log-1",
