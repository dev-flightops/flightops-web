 ...
 *
 * Submits the flip through the `toggleAdminAccessAction` server action
 * (the raw `setAdminAccess` API client is server-only — apiFetch reads
 * the Auth.js cookie, so client components have to route through an
 * action). Optimistic UI; on failure we roll back to the original value
